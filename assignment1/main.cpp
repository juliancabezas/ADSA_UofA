...
std::vector<int> school_multiplication(std::vector<int> number1_vec, std::vector<int> number2_vec, int b) {
    ...
    // Clear the partial results and move the counter forward        
        partial_vec.clear();
        counter = counter + 1;
...
// Utility function to convert the base, only for debugging purposes
...
std::vector<int> karatsuba_multiplication(std::vector<int> number1_vec, std::vector<int> number2_vec, int b) {
    ...
    // In case the vector is less than the separation point
    ...
    // Make recursive calls to get the karatsuba multiplication in the subsets
    ...
    // Now compute the 2 parts of the final ecuation
